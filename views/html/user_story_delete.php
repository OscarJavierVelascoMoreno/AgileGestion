<html>
    <?php
        ...
        if (isset($_POST['delete_user_story'])) {
            if (isset($_GET['usy'])){
                $id = $_GET['usy'];
        
                $query = $connection->prepare("DELETE FROM user_story WHERE id=:id");
                $query->bindParam("id", $id, PDO::PARAM_STR);
                $query->execute();

                header('Location: ./user_story_page.php?usy=1');
            }
        }
                ...
                <div style="width: 100%; height: 90%; display: flex; flex-direction: row;">
                    ...
                    <div style="width: 70%; margin: 5%;">
                        ...
                        <div style="width: 80%; height: 13%; border: 1px solid black; margin: 5%; background-color: #979EA8;">
                            <form method="POST" action="" name="form-delete-user-story">
                                <?php
                                    if (isset($_GET['usy'])) {
                                        $usy_id = $_GET['usy'];
                                        ?>
                                        <button type="submit" name="delete_user_story" value="delete_user_story" style="margin: 5%; border-radius: 5px; background-color: black; color: white; width: 60%">Eliminar</button>
                                        <a href="user_story_page.php?usy=<?php echo $usy_id;?>" class="button">Cancelar</a>
                                        <?php
                                    }
                                ?>
                            </form>
